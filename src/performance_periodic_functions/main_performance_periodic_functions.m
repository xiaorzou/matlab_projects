%generate info for Table 2 of the following papers
% Trigonometric Interpolation on Non-Periodic Functions and its Applications
% 

function main_performance_periodic_functions()
    %
    ...
    output = zeros(length(m_powers)*length(degs),8);    
    for para = paras
        for m_power = m_powers
            ...
            %}
            output(counter,:)=[para, M,  abs_cos, abs_standard,   diff_der_1_cos_true, diff_der_1_standard_true, diff_der_2_cos_true, diff_der_2_standard_true];
            counter = counter+1;
            if do_graph
    ...
    cos_approx_test_cos_file = strcat('output/performance_periodic_functions_', model, '.xlsx');
    vnames = {'deg','M', 'err_fun', 'err_fun_standard',  'err_der1', 'err_der1_standard', 'err_der2', 'err_der2_standard'};
