%
% the function generate Figure 3  Figure 4 of the following paper
% "Trigonometric Interpolation on Non-Periodic Functions and its Applications"
% generate Figure 3 and Figure 4
%

function  main_error_analysis()  
    location = 'northwest';     
    ...
    for model = models
        ...
        file_name = ['output/Fig_4_', model{1}, num2str(round(para)), num2str(q), fig_type];
        plot_latex(file_name, x_plot_this, diff_error, title_this, legend_this, xlabel_this, ylabel_this, location, color_code);

        if strcmp(model, 'default')
            filename = ['output/Fig_3_', num2str(q), fig_type];
