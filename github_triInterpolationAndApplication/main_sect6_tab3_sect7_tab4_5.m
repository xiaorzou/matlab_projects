%generate table 4, 5 in section 7 and table 3 in section 6

function main_sect6_tab3_sect7_tab4_5()
    m_powers = [8];
    plot_size = 2^12;
    %flag_type = 'power';
    %flag_type = 'cos';
    ...
    for flag_type = flag_types
        ...
        for cut_off_para = cut_off_paras
            for delta = deltas
                right = e + delta;
                left = -right;
                %[h_coef_e, ~] = fourier_normalizer_get_h_coef_enhancement(a,b,c,d,n,q_h);
                for m_power = m_powers
                    ...
                    for para = paras
                        ...
                        %f_plot = (1-(x_plot/a).^2).^para;
                        %f_plot_der1 = -(para*2/a^2)*(1-(x_plot./a).^2).^(para-1).*x_plot;
                        %f_plot_der2 = -(para*2/a^2)*(1-(x_plot./a).^2).^(para-1) -para*(para-1)*(1-(x_plot./a).^2).^(para-2).*4/a^4.*x_plot.^2 ;
        ...
        fourier_normalizer_test_file = ['output/table_3_sect6_', flag_type{1}, '.xlsx'];
        ...
        fourier_normalizer_test_int_file = ['output/table_4_5_sect7_', flag_type{1}, '.xlsx'];
