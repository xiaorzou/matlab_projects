...
function fourier_normalizer_app_ode_deg_1_general_test()
    ...
    function val = fun(X)
        ...
        elseif strcmp(task, 'non_linear')
            ...
            y = X(2,:);
            %c = 0.5*pi;
            %f = cos(c*x) - c*x.*sin(c*x)-x.^2.*cos(c*x).*(1+cos(c*x));
            %c = theta;
            %f = deg*x.^(deg-1).*cos(c*x) - c*x.^(deg).*sin(c*x)-x.^(deg+1).*cos(c*x).*(1+x.^(deg-1).*cos(c*x));
    ...
    function val = fun_sol(X)  %only for x<=0
        if strcmp(task, 'linear')
            x = abs(X);
            %val = ((y_s-1)*exp(-(-X.^3-s^3)/3) +1).*co_even; %only for x<=0
    ...
    [x_N, u_N, u_init_N, z_N, z_init_N, pos, opt_error] = fourier_normalizer_app_ode_deg_1_general(@fun, @partial_u, y_s, z_s, s, e, p,q, init_method);
    x_M_plot = x_N(pos);
    u_M_plot = u_N(pos);
    ...
    %handle error
    error_eps_file = ['output/fourier_normalizer_app_ode_deg_1_error_init_', task, '_q_', num2str(q), '_deg_', num2str(deg), '_theta_', num2str(round(theta)), '_init_', init_method ,  figtype_eps];
    error_fig_file = ['output/fourier_normalizer_app_ode_deg_1_error_init_', task, '_q_', num2str(q), '_deg_', num2str(deg), '_theta_', num2str(round(theta)), '_init_', init_method ,  figtype_fig];
    ...
    fourier_normalizer_app_ode_deg_1_general_error_dat_file = ['output/fourier_normalizer_app_ode_deg_1_error_', task, '_q_', num2str(q) , '_deg_', num2str(deg), '_theta_', num2str(round(theta)),'_init_', init_method ,  '.dat'];
    mylib_writearray(vnames, error_output, fourier_normalizer_app_ode_deg_1_general_error_xlsx_file);
    mylib_writearray(vnames, error_output, fourier_normalizer_app_ode_deg_1_general_error_dat_file);
    ...
    fourier_normalizer_app_ode_deg_1_general_ia_der_dat_file = ['output/fourier_normalizer_app_ode_deg_1_ia_der_', task, '_q_', num2str(q) , '_deg_', num2str(deg), '_theta_', num2str(round(theta)),'_init_', init_method ,  '.dat'];
    ...
    mylib_writearray(vnames, ia_der_output, fourier_normalizer_app_ode_deg_1_general_ia_der_dat_file);
    ...
    legend_z = '$rk4$';
    legend_w = '$benc$';
    ia_der_eps_file = ['output/fourier_normalizer_app_ode_deg_1_diff_init_', task, '_q_', num2str(q), '_deg_', num2str(deg), '_theta_', num2str(round(theta)), '_init_', init_method ,  figtype_eps];
    ia_der_fig_file = ['output/fourier_normalizer_app_ode_deg_1_diff_init_', task, '_q_', num2str(q), '_deg_', num2str(deg), '_theta_', num2str(round(theta)), '_init_', init_method ,  figtype_fig];
    ...
    mylib_writearray(vnames, diff_output, fourier_normalizer_app_ode_deg_1_general_ia_der_dat_file);
    ...
    filename_eps = ['output/fourier_normalizer_app_ode_deg_1_uandy_', task, '_q_', num2str(q), '_deg_', num2str(deg), '_theta_', num2str(round(theta)), '_qplot_', num2str(q_plot), '_init_', init_method ,  figtype_eps];
    filename_fig = ['output/fourier_normalizer_app_ode_deg_1_uandy_', task, '_q_', num2str(q), '_deg_', num2str(deg), '_theta_', num2str(round(theta)), '_qplot_', num2str(q_plot), '_init_', init_method ,  figtype_fig];
    ...
    fourier_normalizer_app_ode_deg_1_general_yandu_dat_file = ['output/fourier_normalizer_app_ode_deg_1_yandu_', task, '_q_', num2str(q) , '_deg_', num2str(deg), '_theta_', num2str(round(theta)),'_init_', init_method ,  '.dat'];
    ...
    mylib_writearray(vnames, yandu_output, fourier_normalizer_app_ode_deg_1_general_yandu_dat_file);
    
    %handle performance results and parameters
    fourier_normalizer_app_ode_deg_1_general_max_error_file = ['output/fourier_normalizer_app_ode_deg_1_general_max_error_', task, '_q_', num2str(q), '_deg_', num2str(deg), '_theta_', num2str(round(theta)), '_init_', init_method ,  '.xlsx'];
