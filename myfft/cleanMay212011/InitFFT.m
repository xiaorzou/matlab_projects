...
global N Deg D Left Right Lambda  FFTExp ModelScaling FFTAD FFTF  PInt
global SA SB
...
FFTAD(1,:)=F(1)*(Y-Left)+N*bma*imag(ifft(F1))/pi; %first order anti
...
F1(2:N)=F1(2:N)./K(2:N); %six order
temp5=N*real(ifft(F1));
FFTAD(6,:)=(F(1)/720)*(Y-Left).^6 -bmapi^6*temp5 + bmapi^6*(Y-Left).^0*temp5(N/2+1) - bmapi^4/2*temp3(N/2+1)*(Y-Left).^2 +bmapi^2/24*temp2(N/2+1)*(Y-Left).^4;

if Deg>4
    error('not implemented yet, in InitFFT');
end
MA = 3;
FFTAD_new = zeros(MA*2,N);
Im = zeros (MA, N);
Re = zeros (MA, N);
...
for i=1:MA
    factor = factor *(2*i-1);
    FFTAD_new(2*i-1,:) = (-1)^(i-1)*Im(i,:) + (F(1)/factor)* (Y-Left).^(i*2-1);
    factor_2 = 1;
    for m=1:(i-1)
        factor_2 = factor_2*(2*m-1); 
        FFTAD_new(2*i-1,:) = FFTAD_new(2*i-1,:) + (-1)^(i-m-1)/factor_2*(Y-Left).^(m*2-1)*Re(i-m, N/2+1);
        factor_2 = factor_2 * 2*m ;
    end   
    factor = factor * 2*i;
    FFTAD_new(2*i,:) = (-1)^(i)*Re(i,:) + (F(1)/factor)* (Y-Left).^(i*2);
    factor_2 = 1;
    for m=1:i
        if m ~= 1
            factor_2 = factor_2*(2*m-2);
        end
        FFTAD_new(2*i,:) = FFTAD_new(2*i,:) + (-1)^(i-m)/factor_2*(Y-Left).^(m*2-2)*Re(i-m+1, N/2+1);
        factor_2 = factor_2 *(2*m-1);
    end       
end
...
FFTAD_new = FFTAD_new(:,N/2+1:N);
FFTAD_new = max(FFTAD_new,0);
